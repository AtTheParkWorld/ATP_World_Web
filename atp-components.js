...
(function() {
  ...
  /* ── Footer (Design System v2 · #4) ─────────────────────────────
   * A page renders the unified footer by adding a single mount point:
   *   <div id="atp-footer-host"></div>
   * just before </body>. mountFooter() injects the canonical markup
   * so every page gets the same socials, columns, legal links, and
   * copyright. Pages with their own bespoke footer markup keep
   * working unchanged — this is opt-in. */
  function mountFooter() {
    var host = document.getElementById('atp-footer-host');
    if (!host) return;
    var year = new Date().getFullYear();
    host.innerHTML =
      '<footer role="contentinfo" class="atp-footer">' +
        '<div class="atp-footer-inner">' +
          // ── Single level (founder 2026-09-19) ───────────────────
          // The four stacked link columns + a separate contact/news
          // band made the footer ~880px tall. Everything now sits on
          // ONE row: brand, one inline link list, and a compact
          // newsletter — with a thin legal strip beneath.
          '<div class="atp-footer-row">' +
            '<div class="atp-footer-brand">' +
              '<img loading="lazy" decoding="async" src="' + escapeHtml(LOGO_SRC) + '" alt="At The Park">' +
              '<div class="atp-footer-socials">' +
                '<a class="atp-social-btn" href="https://www.instagram.com/atthepark_world/?hl=en" target="_blank" rel="noopener" aria-label="Instagram"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 5.838c3.405 0 6.162 2.76 6.162 6.162 0 3.405-2.76 6.162-6.162 6.162-3.405 0-6.162-2.76-6.162-6.162 0-3.405 2.76-6.162 6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4z"/></svg></a>' +
                '<a class="atp-social-btn" href="https://www.tiktok.com/@attheparkuae" target="_blank" rel="noopener" aria-label="TikTok"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M19.6 6.3a4.85 4.85 0 0 1-1-.6 6.4 6.4 0 0 1-2.5-4.2h-3.5v14a3 3 0 1 1-2.1-2.9V9a6.6 6.6 0 1 0 5.6 6.6V9.2a8 8 0 0 0 4.8 1.5V7.3a4.6 4.6 0 0 1-1.3-1z"/></svg></a>' +
                '<a class="atp-social-btn" href="https://api.whatsapp.com/send?phone=971585792378" target="_blank" rel="noopener" aria-label="WhatsApp"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981z"/></svg></a>' +
              '</div>' +
            '</div>' +
          ...
          '<div class="atp-footer-bottom">' +
            '<p class="atp-footer-copy">&copy; ' + year + ' At The Park &middot; Dubai &middot; Al Ain &middot; Abu Dhabi &middot; Muscat</p>' +
            '<nav class="atp-footer-legal" aria-label="Legal">' +
  ...
  /* ── Empty + Loading helpers (Design System v2) ─────────────────
   * Used to be local to profile.html. Hoisted here so every page can
   * call `_atpEmpty(...)` / `_atpSpinner(...)` and get the same
   * canonical block. Pages that already roll their own can adopt
   * these on next edit. */
  function _atpEmpty(icon, title, desc, ctaHtml) {
    var safe = function(s){ return String(s == null ? '' : s); };
    return '<div class="atp-empty">' +
      '<div class="atp-empty__icon">' + safe(icon) + '</div>' +
      '<h3 class="atp-empty__title">' + safe(title) + '</h3>' +
      (desc ? '<p class="atp-empty__desc">' + safe(desc) + '</p>' : '') +
      (ctaHtml ? '<div class="atp-empty__cta">' + ctaHtml + '</div>' : '') +
    '</div>';
  }
  function _atpSpinner(label) {
    return '<div class="atp-empty" role="status" aria-live="polite">' +
      '<div class="atp-spinner atp-spinner--lg" aria-hidden="true"></div>' +
      (label ? '<p class="atp-empty__desc" style="margin:0">' + String(label) + '</p>' : '') +
    '</div>';
  }
  // Make the helpers globally callable without changing call sites.
  window._atpEmpty   = _atpEmpty;
  window._atpSpinner = _atpSpinner;
